...
var hwcgi='https://api.translate.shoky.ru/goqhanzi';var drawkanji;function message(text)
...
{this.canvas=new DrawCanvas();this.found_kanji=getbyid("found_kanji");this.adjust_canvas_offsets();var self=this;this.canvas.element.onmouseup=function(event){self.mouseup(event);}
...
if(this.stroke_num>=53){
    message(ml.too_many_strokes);throw"Too many strokes";
}
var key="H";
if(this.ignore_stroke_order){key="h";}
var r=this.makeMessage(key);
var self=this;
sendit(r,function(result){self.result_callback(result)},hwcgi);
if(this.has_local_storage){localStorage.kanji=r;}}
...
function get_touch_position(event) {
    ...
    var x = touchobj.clientX; // Используем clientX
    var y = touchobj.clientY; // Используем clientY
    return { "x": x, "y": y };
}
DrawKanji.prototype.touch_start = function(event) {
